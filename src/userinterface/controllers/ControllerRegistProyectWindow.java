...
public class ControllerRegistProyectWindow implements EventHandler<ActionEvent> {
    ...
    private boolean validateAllFields() {
        ...
        if (!validators.validateDate(view.getDateStartTextField().getText())) {
            showError("Fecha de inicio inválida o formato incorrecto (YYYY-MM-DD)");
            highlightField(view.getDateStartTextField());
            isValid = false;
        }

        if (!validators.validateDate(view.getDateEndTextField().getText())) {
            showError("Fecha de fin inválida o formato incorrecto (YYYY-MM-DD)");
